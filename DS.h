...
template <class T, class Compare>
void HVector<T, Compare>::sort() {
    int originalSize = cur;
...
template <class T, class Compare>
T HVector<T, Compare>::pop(int ind) {
    if (ind < 0 || ind >= cur) {
        throw std::out_of_range("Index out of bounds");
    }

    T removedElement = arr[ind];  // Get the element at the specified index
...
T HVector<T, Compare>::get(int ind) {
    if (ind >= 0 && ind < cur) {
        return arr[ind];                // Return the element at the specified index
    }
    return nullptr;
}
